__all__ = ("QSONumberCountsMetric_gdli",)
...
class QSONumberCountsMetric_gdli(BaseMetric):
    ...
    def run(self, data_slice, slice_point=None):
        ...
        f = open(f'm_range_{self.lsst_filter}.txt','a+')
        f.write(str(m_bright))
        f.write(',')
        f.write(str(mlim5))
        f.write(',')
        f.write(str(pix_area))
        f.write(',')
        f.write(str(nqso[0]))
        f.write('\n')
        return nqso
        ...
        if self.mlim<=m_bright:
            return np.array([0])
        elif self.mlim>m_bright and self.mlim<mlim5:
            mlim = self.mlim
            n11 = self.nqso_cumulative(self.zmin, m_bright)
            n12 = self.nqso_cumulative(self.zmin, mlim)
            n21 = self.nqso_cumulative(self.zmax, m_bright)
            n22 = self.nqso_cumulative(self.zmax, mlim)
            nqso = (n22 - n21 - n12 + n11) * pix_area
            return nqso
        elif self.mlim>=mlim5:
            mlim = mlim5
            n11 = self.nqso_cumulative(self.zmin, m_bright)
            n12 = self.nqso_cumulative(self.zmin, mlim)
            n21 = self.nqso_cumulative(self.zmax, m_bright)
